...
def test_mapCorrelation():
    ...
    ranges = np.load(path+"test_ranges.npy")
    # take valid indices
    ...
    # x_range = np.arange(0,0.4+0.05,0.05)
    # y_range = np.arange(0,0.4+0.05,0.05)
...
def occupancy_grid_plot(coordinates):
    # coordinates in physical units (in meters) of occupied cells
    # init MAP
    MAP = {}
    MAP['res']   = 0.05 #meters
    MAP['xmin']  = -45  #meters
...
def vel_theta(vel,theta,time_value):
    figure, axis = plt.subplots(3, 1,figsize=(16, 6),layout = 'constrained')
    axis[0].plot(time_value[:-1], vel[:,0],color='g', label='last')
    axis[0].set_title("velocity x")
    axis[1].plot(time_value[:-1], vel[:,1],color='g', label='last')
    axis[1].set_title("velocity y")
    axis[2].plot(time_value[:-1], theta,color='g', label='last')
    axis[2].set_title("theta")
    plt.show
    return figure
